...
void Default_Handler(void) {
    while (1) {
        // Infinite loop when handler is called
    }
}

// Reset handler
void Reset_Handler(void) {
    // Copy .data section contents from flash to SRAM
    // Initialize .bss section (uninitialized global/static variables) in SRAM to zero

    // Call board-specific initialization function if needed

    // Call main function (assuming it is defined somewhere)
